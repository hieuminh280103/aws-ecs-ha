...
resource "aws_lb_target_group" "ecs_doc" {
  name     = "${local.name_prefix}-ecs-doc-tg"
  port     = 3000
  protocol = "HTTP"
  vpc_id   = module.vpc.vpc_id
...
resource "aws_lb_listener" "http_forward_to_tg" {
  load_balancer_arn = aws_lb.ecs_alb.arn
  port              = 80
  protocol          = "HTTP"
  default_action {
...
resource "aws_lb_listener_rule" "api" {
  ...
  listener_arn = aws_lb_listener.http_forward_to_tg.arn
  priority     = 2
  action {
    type             = "forward"
...
#Forward to doc tg
resource "aws_lb_listener_rule" "doc" {
  # listener_arn = aws_lb_listener.https_forward_to_tg.arn
  listener_arn = aws_lb_listener.http_forward_to_tg.arn
  priority     = 3
  action {
